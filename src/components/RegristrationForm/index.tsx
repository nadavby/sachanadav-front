...
const {ref,...rest} = register('img');
    return(
        ...
        <img src={file?URL.createObjectURL(file):avatar} alt="" style={{width:'200px',height:'200px',alignSelf:'center',borderRadius:'50%',marginBottom:'10px'}}/>
        <FontAwesomeIcon icon={faImage} className="fa-xl " onClick={()=>{inputFileRef.current?.click()}} style={{alignSelf:'baseline',marginBottom:'20px',marginLeft:'275px'}}/>
        <input  {...rest} ref={(e)=>{ref(e); inputFileRef.current=e}} id='img' type="file"className="mb-3" accept="image/jpeg,image/png" style={{display:'none'}}/>
        <label>email:</label>
        <input {...register('email')} type="email" placeholder="Enter your email" className="mb" style={{borderRadius:'5px',border:'1px'}}/>
        {errors.email && <p className='text-danger'>{errors.email.message}</p>}
        <label>password:</label>
        <input {...register('password')} type="password" placeholder="Enter your password" className="mb-3" style={{borderRadius:'5px',border:'1px'}}/>
